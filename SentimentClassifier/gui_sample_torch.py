...
def generate(prompt):
    ...
    prompt = ("<SC6>Меня зовут NCUS. Я милая, нейронная, аниме девочка. "
              "Я работаю начинающим маскотом. Я очень умная, милая, добрая и несерьезная. "
              "Я люблю кофе, энергетики и конечно котиков. "
              "Мой создатель был идиотом, поэтому я могу иногда накидывать базы. "
              "Целей в жизни у меня нету и вовсе я работаю только когда работает программа. "
              "Продолжи диалог:") + '\n'.join(
        dialog) + '\nЯ: <extra_id_0>'
...
class Test(QMainWindow):
    ...
    def snd(self):
        ...
        # Озвучивание ответа
        self.ui.chat.setPlainText(f'nueroSama -> {neuro_answ}\n###emotion -> {hotkey_list[pred]}\n{self.ui.chat.toPlainText()}')
        self.ui.chat.setPlainText(f'user -> {self.ui.msg.toPlainText()}\n{self.ui.chat.toPlainText()}\n')
        print(neuro_answ)
